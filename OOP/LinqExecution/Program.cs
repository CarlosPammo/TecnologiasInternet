	...
	class Program
	{
		static void Main(string[] args)
		{
			...
			var careers = new CareerDataAccess()
				.GetCareeers();
            var stuCarSub = new StuCarSub()
                .GetStuCarSub();
            var subjects = new SubjectDataAccess()
                .GetSubject();
            ...
            //            career
            //        })
            //    .Where(joined => joined.career.Name == "Ing. de Sistemas"&& joined.student.Birthdate.Month==4)
            //    .Select(joined => joined.student);

            //Estudiantes de Calculo
            var filtered =
                from student in students
                join study in stuCarSub on student.ID equals study.IdStudent
                join career in subjects on study.IdCareer equals career.ID

                where career.Name == "Algebra I" 
                select student;
